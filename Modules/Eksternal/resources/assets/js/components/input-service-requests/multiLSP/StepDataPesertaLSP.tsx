...
import { ParticipantLSP, emptyParticipantLSP } from "../../../types/lsp"
import FormDataPesertaLSP from "./FormDataPesertaLSP"
...
const TabBar = styled.div`
  ...
  flex-wrap: wrap; border-bottom: 2px solid #dee2e6; margin-bottom: 1.5rem;
`
...
const AddTabBtn = styled.button`
  ...
  border: 1px dashed #adb5bd; border-radius: 8px 8px 0 0;
  background: transparent; color: #6c757d; font-size: 0.8rem;
  cursor: pointer; position: relative; bottom: -2px; transition: all 0.15s;
...
interface Props {
  participants: ParticipantLSP[]
  setParticipants: React.Dispatch<React.SetStateAction<ParticipantLSP[]>>
  nextId: number
...
const StepDataPesertaLSP: React.FC<Props> = ({
  ...
  const addParticipant = () => {
    const newP = emptyParticipantLSP(nextId)
  ...
  return (
    <div>
      ...
      {/*  Semua peserta di-render, yang tidak aktif display:none */}
      {participants.map((p) => {
...
export default StepDataPesertaLSP