...
<div class="space-y-4">
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="kal-stat-card border-start border-3 border-primary">
                <span class="text-uppercase text-muted" style="font-size: 11px; letter-spacing: 0.5px;">Total Alat Dikalibrasi</span>
                <h4 class="fw-bold text-primary mb-0 mt-1">
                    {{ $form->total_alat ?? $form->alatList->count() }}
                    <small class="fs-6 text-muted fw-normal">Unit Alat</small>
                </h4>
            </div>
        </div>
        <div class="col-md-4">
            <div class="kal-stat-card border-start border-3 border-success">
                <span class="text-uppercase text-muted" style="font-size: 11px; letter-spacing: 0.5px;">Estimasi Total Biaya</span>
                <h4 class="fw-bold text-success mb-0 mt-1">
                    Rp {{ number_format($form->estimasi_total_tarif ?? 0, 0, ',', '.') }}
                </h4>
            </div>
        </div>
        <div class="col-md-4">
            <div class="kal-stat-card border-start border-3 border-info">
                <span class="text-uppercase text-muted" style="font-size: 11px; letter-spacing: 0.5px;">Lokasi Pelaksanaan</span>
        ...
        @forelse($form->alatList as $index => $alat)
            <div class="kal-alat-card shadow-sm">
                <div class="kal-alat-header d-flex justify-content-between align-items-center">
                    <div>
                        <span class="badge bg-primary me-2">Alat #{{ $index + 1 }}</span>
                ...
                <div class="p-3">
                    ...
                    @if($alat->nomorSeriList && $alat->nomorSeriList->isNotEmpty())
                        <div class="mb-3 p-2 rounded bg-light border">
                            <small class="text-muted fw-bold d-block mb-1">Nomor Seri Unit:</small>
                            <div class="d-flex flex-wrap gap-2">
                                @foreach($alat->nomorSeriList as $seri)
                                    <span class="badge bg-white text-dark border">
                                        Unit {{ $seri->nomor_unit }}: <strong>{{ $seri->nomor_seri ?: '-' }}</strong>
                                    </span>
                                @endforeach
                            </div>
                        </div>
                    ...
                    @if($alat->kalibrasiItems && $alat->kalibrasiItems->isNotEmpty())
                        <div>
                            <small class="text-muted fw-bold d-block mb-2">Item / Parameter Tarif:</small>
    ...
    <div class="p-3 bg-light rounded border">
        <div class="d-flex align-items-center gap-2">
            @if($form->setuju_pernyataan)
                ...
                <div>
                    <div class="fw-semibold text-success">Pemohon telah menyetujui Ketentuan & Pernyataan Layanan Kalibrasi</div>
                    <small class="text-muted">
                        Disetujui pada: {{ $form->pernyataan_at ? \Carbon\Carbon::parse($form->pernyataan_at)->format('d F Y, H:i') . ' WIB' : '-' }}
                    </small>
                </div>
            @else
